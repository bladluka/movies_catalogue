...
@app.route('/')
def homepage():
    type_list = ['top_rated', 'upcoming', 'popular', 'now_playing']
    selected_list = request.args.get('list_type', 'popular')
    if selected_list not in type_list:
        selected_list = type_list[2]
    movies = tmdb_client.get_movies(how_many=8, list_type=selected_list)
    return render_template("homepage.html", movies=movies, selected_list=selected_list, type_list=type_list)
